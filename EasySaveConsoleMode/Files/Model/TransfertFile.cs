...
namespace EasySaveConsole.Model
{
    class TransferFile
    {
        ...
        private void MoveFile()
        {
            ...
            this.TotalFilesToCopy = this.filesOrigin.Length;
            ...
            foreach (string file in this.filesOrigin)
            {
                ...
                this.Progression = int.Parse((this.TotalFilesToCopy * (this.TotalFilesToCopy - this.NbFilesLeftToDo) / 100).ToString());
                this._state[this._ID].NbFilesLeftToDo = this.NbFilesLeftToDo;
                this._state[this._ID].Progression = this.Progression;
                this._state[this._ID].State =  (this._state[this._ID].NbFilesLeftToDo == 0) ?  "END" : "ACTIV";
                this._state[this._ID].TotalFilesToCopy = TotalFilesToCopy;
                this._state[this._ID].Name = this._name;
                try
                {
                    File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + "\\Log\\StateLog\\State.json", JsonSerializer.Serialize(this._state, new JsonSerializerOptions { WriteIndented = true }));
                }
                catch
                {
                }
